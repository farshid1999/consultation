...
import { useAdminSubmissions } from "@/hooks/useAssignment";
...
export default function AdminAssignmentSubmissionsPage() {
  ...
  // فرض بر این است که بک‌اند شما امکان فیلتر کردن سابمیشن‌ها بر اساس assignment_id را از طریق کوئری پارامتر می‌دهد
  const { data: submissionsData, isLoading } = useAdminSubmissions({ 
    assignment: assignmentId, // یا assignment_id بسته به نام فیلد در بک‌اند
    page: 1 
  });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-cream">پاسخ‌های دریافت‌شده</h1>
          <p className="text-cream/50 text-sm mt-1">
            {submissionsData?.pagination?.count ?? 0} پاسخ برای این تکلیف ثبت شده است
          </p>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-white/5 bg-white/[0.02] backdrop-blur-sm overflow-hidden">
        {submissionsData?.results?.length === 0 ? (
          <div className="text-center py-16 text-cream/40 text-sm">
            هنوز هیچ عضوی به این تکلیف پاسخ نداده است.
          </div>
        ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-cream/40 text-xs mt-1">
                      <FiClock size={12} />
                      <span>ارسال در: {formatJalaliDateTime(new Date(submission.created_at))}</span>
                    </div>
                  </div>
                </div>

                {/* لینک به صفحه جزئیات پاسخ (همان صفحه چت که قبلاً ساختیم) */}
                <Link
                  href={`/admin/assignments/${assignmentId}/submissions/${submission.id}`}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gold/10 text-gold text-sm font-medium hover:bg-gold/20 transition-all border border-gold/20 group-hover:border-gold/40"
                >
                  <FiEye size={14} />
                  مشاهده و گفتگو
                </Link>
