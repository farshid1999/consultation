        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-cream/10 bg-cream/[0.03]">
              ...
              <th className="text-right px-5 py-3.5 text-cream/50 font-medium">لاین</th>
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={4} className="text-center py-12 text-cream/30 text-sm">در حال بارگذاری...</td>
              </tr>
            ) : data?.results?.length === 0 ? (
              <tr>
                <td colSpan={4} className="text-center py-12 text-cream/30 text-sm">تکلیفی یافت نشد</td>
              </tr>
            ) : (
              data?.results?.map((assignment) => (
                <tr key={assignment.id} className="border-b border-cream/5 hover:bg-cream/[0.02] transition-colors">
                  <td className="px-5 py-4 text-cream font-medium">{assignment.title}</td>
                  <td className="px-5 py-4 text-cream/60">{assignment.line}</td>
                  <td className="px-5 py-4 text-cream/50 max-w-xs truncate">{assignment.description || "—"}</td>
