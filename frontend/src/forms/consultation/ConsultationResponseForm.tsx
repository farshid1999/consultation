...
function buildResponseFormData(values: ResponseFormValues, keptFileIds: string[]): FormData {
  const formData = new FormData();
  formData.append("title", values.title);
  if (values.description) formData.append("description", values.description);

  keptFileIds.forEach((id) => formData.append("existing_file_ids", id));
...
export default function ConsultationResponseForm() {
  ...
  const onSubmit = async (values: ResponseFormValues) => {
    ...
    const fd = buildResponseFormData(values, keptFileIds);
